...
public class PreviewTagMergeTests
{
    ...
    public async Task ExecuteAsync_NoOverlap_ReturnsSumOfCounts()
    {
        ...
        var sourceTask1 = TaskItem.CreateStandalone(_userId, "ST1");
        var sourceTask2 = TaskItem.CreateStandalone(_userId, "ST2");
        ...
        var sourceNote = Note.Create(_userId);
        ...
        var targetTask = TaskItem.CreateStandalone(_userId, "TT1");
        _taskRepo.GetTasksWithTagAsync(_userId, _targetTagId, Arg.Any<CancellationToken>())
            .Returns(new List<TaskItem> { targetTask });

        var targetNote1 = Note.Create(_userId);
        var targetNote2 = Note.Create(_userId);
        _noteRepo.GetByTagIdAsync(_userId, _targetTagId, Arg.Any<CancellationToken>())
            .Returns(new List<Note> { targetNote1, targetNote2 });

        var targetMeeting = Meeting.Create(_userId, "TM1", attendees: "");
        _meetingRepo.GetByTagIdAsync(_userId, _targetTagId, Arg.Any<CancellationToken>())
    ...
    public async Task ExecuteAsync_PartialOverlap_ReturnsCorrectResultAndOverlapCount()
    {
        ...
        var sharedTask = TaskItem.CreateStandalone(_userId, "Shared");
        var sourceOnlyTask = TaskItem.CreateStandalone(_userId, "Source Only");
    ...
    public async Task ExecuteAsync_SourceHasNoItems_ReturnsTargetCountsUnchanged()
    {
        ...
        var targetTask1 = TaskItem.CreateStandalone(_userId, "TT1");
        var targetTask2 = TaskItem.CreateStandalone(_userId, "TT2");
        _taskRepo.GetTasksWithTagAsync(_userId, _targetTagId, Arg.Any<CancellationToken>())
            .Returns(new List<TaskItem> { targetTask1, targetTask2 });

        var targetNote = Note.Create(_userId);
        _noteRepo.GetByTagIdAsync(_userId, _targetTagId, Arg.Any<CancellationToken>())
