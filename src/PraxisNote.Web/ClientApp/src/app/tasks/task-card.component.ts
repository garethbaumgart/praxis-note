...
import { TaskCommentsSectionComponent } from './task-comments-section.component';
...
@Component({
  ...
  template: `
    <div
      class="bg-surface-subtle rounded-md py-2 px-3 border transition-colors task-highlight-target group"
    ...
    >
      ...
      <div class="flex items-start gap-2">
        ...
        @if (commentsExpanded()) {
          <app-task-comments-section
            #commentsSection
            [comments]="task().comments"
            (onAdd)="onAddComment.emit($event)"
            (onEdit)="onEditComment.emit($event)"
            (onDelete)="onDeleteComment.emit($event)"
          />
        }
...
export class TaskCardComponent {
  ...
  // Comments section child component
  readonly commentsSection = viewChild<TaskCommentsSectionComponent>('commentsSection');
  ...
  toggleTab(tab: 'dueDate' | 'comments' | 'tags'): void {
    ...
    if (currentTab === tab) {
      ...
      if (tab === 'comments') {
        afterNextRender(() => {
          this.commentsSection()?.focusNewCommentInput();
        }, { injector: this.injector });
      }
    }
  }
