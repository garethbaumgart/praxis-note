                          ...
                          <div class="flex flex-wrap items-center gap-1 mt-1.5" (click)="$event.stopPropagation()">
                            ...
                            @if (!meeting.isDuplicate) {
                              <input
                                ...
                                placeholder="+ tag"
                                [attr.aria-label]="'Add tag to ' + (meeting.title || 'Untitled Meeting')"
                                (keydown.enter)="$any($event.target).value.trim() && transcriptService.addTag(meetingIdx, $any($event.target).value.trim()); $any($event.target).value = ''"
