    ...
    IMeetingRepository meetingRepository)
{
    ...
    public async Task<IReadOnlyList<TagDto>> ExecuteAsync(Query query, CancellationToken cancellationToken = default)
    {
        ...
        var taskCounts = await taskRepository.GetTagUsageCountsAsync(query.UserId, cancellationToken);
        var noteCounts = await noteRepository.GetTagUsageCountsAsync(query.UserId, cancellationToken);
        var meetingCounts = await meetingRepository.GetTagUsageCountsAsync(query.UserId, cancellationToken);
