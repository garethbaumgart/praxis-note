...
public sealed class UpdateMeetingExcludeFromInsights(IMeetingRepository meetingRepository, IUnitOfWork unitOfWork)
{
    public record Command(Guid MeetingId, Guid UserId, bool Exclude);
    ...
    public async Task<bool> ExecuteAsync(Command command, CancellationToken cancellationToken = default)
    {
        ...
        meeting.SetExcludeFromInsights(command.Exclude);
